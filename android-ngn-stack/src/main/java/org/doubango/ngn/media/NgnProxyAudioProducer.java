...
public class NgnProxyAudioProducer extends NgnProxyPlugin {
    ...
    private int prepareCallback(int ptime, int rate, int channels) {
        Log.i(NgnProxyAudioProducer.TAG, "prepareCallback(" + ptime + ","
                + rate + "," + channels + ")");
    ...
    private synchronized int prepare(int ptime, int rate, int channels) {
        ...
        mRate = rate;
        mChannels = channels;
        Log.i(TAG, "Configure aecEnabled:" + aecEnabled);
        int audioSrc = MediaRecorder.AudioSource.CAMCORDER;
...
//            audioSrc = MediaRecorder.AudioSource.VOICE_RECOGNITION;
        ...
        Log.i(TAG, "prepare audioSrc: " + audioSrc);
        ...
        try {
            ...
            Log.e(TAG, "prepare error: " + ex.getMessage());
            super.mPrepared = false;
            return -1;
        }
    }
        ...
        public void run() {
            ...
            if (mHasBuiltInAEC) {
                ...
                if (ngnAVSession != null) {
                    ngnAVSession.setAECEnabled(true);
                }
            }
