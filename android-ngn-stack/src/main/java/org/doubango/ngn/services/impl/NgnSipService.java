...
public class NgnSipService extends NgnBaseService implements INgnSipService,
    ...
    public boolean register(Context context) {
        ...
        Log.i(TAG, String.format("realm='%s', impu='%s', impi='%s'",
        ...
        if (mConfigurationService.getBoolean(
            ...
            Log.i(TAG, "STUN domain: " + String.format("_stun._udp.%s", domain));
            ...
            Log.i(TAG, String.format("STUN1 - server=%s and port=%d",
                    server, port[0]));
            boolean setSTUN = mSipStack.setSTUNServer(server, port[0]);// Needed event if null
            Log.i(TAG, "setSTUNServer result: " + setSTUN);
        } else {
            ...
            Log.i(NgnSipService.TAG, String.format(
        ...
        Log.i(TAG, String.format("Local IP='%s'", mPreferences.getLocalIP()));
        ...
        Log.i(TAG, String.format("TLS - pvk='%s' pbk='%s' ca='%s' verify=%s",
                    ...
                    /* ==== 3GPP SMSIP === */
                        ...
                        if (smsData != null) {
							...
							/* === We have received any non-RP-DATA message === */
								 ...
								 * update it's status
								 */
                                    Log.i(NgnSipService.TAG, "RP-ACK");
                                } else if (smsType == twrap_sms_type_t.twrap_sms_type_error) {
								/*
								 * Find message from the history (by MR) an
								 * update it's status
								 */
                                    Log.i(NgnSipService.TAG, "RP-ERROR");
                                }
                            }
                        }
                    } else {
        ...
        public int OnStackEvent(StackEvent e) {
            ...
            switch (code) {
                case tinyWRAPConstants.tsip_event_code_stack_started:
                    ...
                    Log.i(NgnSipService.TAG, "Stack started");
                    break;
